import { readTextFile, writeTextFile, exists, readDir, mkdir, BaseDirectory } from '@tauri-apps/plugin-fs';
...
class TauriFileSystemManager {
  ...
  async createClaudeDirectory(): Promise<void> {
    try {
      ...
      await writeTextFile(`${claudePath}/config.json`, JSON.stringify(defaultConfig, null, 2));
    } catch (error) {
  ...
  async createCodexDirectory(): Promise<void> {
    try {
      ...
      await writeTextFile(`${codexPath}/config.json`, JSON.stringify(defaultConfig, null, 2));
    } catch (error) {
      ...
      for (const entry of entries) {
        if (entry.isFile && entry.name.endsWith('.md')) {
          try {
            ...
            const prompt = this.parseMarkdownPrompt(content, entry.name, promptsPath);
            if (prompt) {
              prompts.push(prompt);
            }
          } catch (error) {
   ...
   * 解析Markdown格式的命令文件
   */
  private parseMarkdownPrompt(content: string, filename: string, basePath: string): CustomPrompt | null {
    try {
      ...
      const commandContent = frontmatterEnd > 0 
  ...
  async installCodex(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      return { success: false, message: `安装失败: ${(error as Error).message}` };
    }
  }
